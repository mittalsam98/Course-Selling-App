...
export default function SideBar() {
  const getMenuRow = (
    ...
    if (showInUserLoggedIn) {
      if (!isAuthenticated()) {
        return false;
      }
    }
    if (showInAdminLoggedIn) {
  ...
  return (
    <aside id='separator-sidebar' class='w-64 fixed h-screen' aria-label='Sidebar'>
      <div class='px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800 h-full'>
        <ul class='space-y-2'>
          {getMenuRow(<Home className='h-4 w-4' />, 'Home', true, false, false)}
          {getMenuRow(<Courses className='h-4 w-4' />, 'Courses', true, false, false)}
          {getMenuRow(<Purchase className='h-4 w-4' />, 'Purchases', false, true, false)}
        </ul>
        <ul class='pt-4 mt-4 space-y-2 border-t border-gray-200 dark:border-gray-700'>
          {getMenuRow(<Logout className='h-4 w-4' />, 'Logout', true, true, true)}
          {getMenuRow(<Help className='h-4 w-4' />, 'Help', true, false, false)}
        </ul>
      </div>
    </aside>
  );
